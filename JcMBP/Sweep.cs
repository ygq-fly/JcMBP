...
namespace JcMBP
{
    ...
    public abstract class Sweep
    {
        ...
        bool AdjustPower(ref double d1, ref double d2)
        {
            double disp1 = 0;
            double disp2 = 0;
            ...
            ClsJcPimDll.HwSetCoup(ClsJcPimDll.JC_COUP_TX1);
        ...
        public void Ini()
        {
            ...
            double dd1 = 0, dd2 = 0;
            if (!AdjustPower(ref dd1, ref dd2))
                return;
        ...
        public abstract void SweepTest2(double dd1, double dd2);
    ...
    public class SweepTime : Sweep
    {
        ...
        public override void SweepTest2(double dd1, double dd2)
        {
            double get_xnum = 0;
            if (ClsUpLoad.sm == SweepMode.Poi || ClsUpLoad.sm == SweepMode.Np)
            {
                ...
                {
                    ClsJcPimDll.fnSetTxOn(false, ClsJcPimDll.JC_CARRIER_TX1TX2);//关闭功放
                    MessageBox.Show("互调频率不在rx接收范围内  POI_rx= " + get_xnum.ToString());
                    return;
                }
            }
            else
            {
                ...
                {
                    ClsJcPimDll.fnSetTxOn(false, ClsJcPimDll.JC_CARRIER_TX1TX2);//关闭功放
                    MessageBox.Show("互调频率不在rx接收范围内 HW_rx= " + get_xnum.ToString());
                    return;
                }
            }
    ...
    public class SweepFreq : Sweep
    {
        ...
        public override void SweepTest2(double dd1, double dd2)
        {
            ...
            for (int i = 0; i <= n1; i++)
            {
                ...
                {
                    //设置功率
                    s = ClsJcPimDll.fnSetTxPower(ds.pow1, ds.pow2, ds.freq_off1 + dd1, ds.freq_off2 + dd2);
                    if (s <= -1000)
                    {
                        MessageBox.Show("setpow err");
                    }
                    //设置频率
                    s = ClsJcPimDll.HwSetTxFreqs(f, ds.freq2e, "mhz");
                    if (s <= -10000)
                    {
                        ClsJcPimDll.fnSetTxOn(false, ClsJcPimDll.JC_CARRIER_TX1TX2);//关闭功放
                        PowHandmethod(s);
            ...
            for (int i = 0; i <= n2; ++i)
            {
                ...
                //设置功率
                ClsJcPimDll.fnSetTxPower(ds.pow1, ds.pow2, ds.freq_off1 + dd1, ds.freq_off2 + dd2);
                //设置频率
                s = ClsJcPimDll.HwSetTxFreqs(ds.freq1s, f, "mhz");
                if (s <= -10000)
                {
                    ClsJcPimDll.fnSetTxOn(false, ClsJcPimDll.JC_CARRIER_TX1TX2);//关闭功放
                    PowHandmethod(s);
